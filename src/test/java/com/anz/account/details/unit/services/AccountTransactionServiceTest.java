...
public class AccountTransactionServiceTest {
	...
	private List<AccountTransaction> createAccountTxnMockData() {
		return Arrays.asList(AccountTransaction.builder().id("12").accountNumber("123587")
				.accountName("TestSavingsAccount").creditAmount("0").debitAmount("125.25").currency("SGD")
				.debitCreditIndicator(DebitCreditIndicator.Credit).transactionNarrative(null)
				.valueDate("2021-12-12 00:00:00").build());
	}
